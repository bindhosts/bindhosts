...
export function setupSwipeToClose(element) {
    ...
    const handleMove = (e) => {
        ...
        // Disable swipe in opposite direction
        if (deltaX < 0) return;
        ...
        if (currentX * multiplier < 50) return;

        const bodyContent = document.querySelector('.body-content[data-active="true"]');
        const adjustedX = (Math.abs(currentX) - 50) * multiplier;
        ...
        const opacity = 1 - (Math.abs(adjustedX) / window.innerWidth);
