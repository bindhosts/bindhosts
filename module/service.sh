...
if [ -w $target_hostsfile ] ; then
	...
	string="description=status: ready 🚀" && update_desc=true
	# theres no need to re-update when it was already updated
	grep -q "# bindhosts v" $target_hostsfile && update_desc=false
	# read out if Adaway did something 
	grep -q "generated by AdAway" $target_hostsfile && update_desc=true && string="description=status: active ✅ | 🛑 AdAway 🕊️"
	# if it was already written with adaway, dont update
	grep -q "AdAway" $MODDIR/module.prop && update_desc=false
else
	string="description=status: failed 😭 needs correction 💢" && update_desc=true
...
if [ "$update_desc" = true ]; then
	busybox sync
	busybox sed -i "s/^description=.*/$string/g" $MODDIR/module.prop
	busybox sync
fi
